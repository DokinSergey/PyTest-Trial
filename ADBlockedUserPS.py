...
###########################################################################################################################################
def GetADBlockedUser(dm1:str = '1more',dm0:str = 'cloud',dbg:bool = False)->None:
# def get_UserAD()->tuple[dict[str,str]]:
    ...
    try:
        ...
        with Connection(ADServer, user = r'1more.cloud\script4service', password = r'MhNn@wQDIYmZ',
            ...
            conn.search(f'dc={dm1},dc={dm0}', f"(&(objectclass=Group)(cn={ADuser}))",attributes=ALL_ATTRIBUTES)
    ...
    ##--------------------------------------------------------------------------------------------
        ...
        if dbg and bool(_res):
            for _gr in _res:
                ...
                rpn(f'          company : {_gr.get('company','<*>')}')
                rpn(f'          company : {_gr.get('description','<*>')}')
                # rpn(f'          Enabled : {_gr['Enabled']}')
                rpn(f'   accountExpires : {_gr.get('accountExpires','<*>')}')
